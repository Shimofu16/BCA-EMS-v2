            ...
            <thead>
                ...
                <tr>
                    <th colspan="4" class="fw-bold text-center">
                        <h4 class="mb-1 mt-0">ENROLLED STUDENT LISTS</h4>
            ...
            <tbody>
                @foreach ($students as $student)
                    <tr>
                        ...
                        <td class="bordered-body">{{ Str::ucfirst($student->last_name) }},
                            {{ Str::ucfirst($student->first_name) }}
                            {{ Str::ucfirst($student->middle_name) }}</td>
                        <td class="bordered-body">{{ date('M d, Y', strtotime($student->updated_at)) }}</td>
                        <td class="bordered-body">{{ $student->department }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
